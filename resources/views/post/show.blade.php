            ...
            <div class="row">
                <div class="col-lg-9 mx-auto">
                        ...
                        @auth()
                            <form action="{{ route('post.like.store', $post->id) }}" method="POST">
                                ...
                                <button type="submit" class="border-0 bg-transparent">
                                    <i class="nav-icon fa{{ auth()->user()->likedPosts->contains($post->id) ? 's' : 'r' }} fa-solid fa-heart"></i>
                                </button>
                            </form>
                        @endauth
                        @guest()
                            <div>
                                <span>{{ $post->liked_users_count }}</span>
                                <i class="nav-icon far fa-regular fa-heart"></i>
                            </div>
                        @endguest
                    </section>
                    ...
                    <section class="comment-list mb-5">
                        <h2 class="section-title" data-aos="fade-up"> Комментарии ({{ $post->comments->count() }})</h2>
                    ...
                    @auth()
                        <section class="comment-section" style="margin-bottom: 150px;">
